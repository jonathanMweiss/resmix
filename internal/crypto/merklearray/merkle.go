...
func Build(array Array, w crypto.BWriter) (*Tree, error) {
	tree := &Tree{}
	...
	for i := uint64(0); i < arraylen; i++ {
		...
		leaves = append(leaves, crypto.HashObj(data, w))
	}
	...
	if arraylen > 0 {
		...
		for len(tree.topLayer()) > 1 {
			tree.levels = append(tree.levels, tree.topLayer().up(w))
		}
	}
...
func (tree *Tree) Prove(idxs []uint64, w crypto.BWriter) ([]crypto.Digest, error) {
	if len(idxs) == 0 {
		return nil, nil
	}
	...
	for l := uint64(0); l < uint64(len(tree.levels)-1); l++ {
		var err error
		pl, err = pl.up(s, l, validateProof, w)
		if err != nil {
			return nil, err
		}
	}
...
func Verify(root crypto.Digest, elems map[uint64]crypto.Hashable, proof []crypto.Digest, w crypto.BWriter) error {
	if len(elems) == 0 {
		if len(proof) != 0 {
