...
type Hashable interface {
	ToBeHashed(BWriter) (HashID, []byte)
}

type BWriter interface {
	// allows to write into this buffer.
	io.Writer
	// outputs the slice pointing to the underlying array.
	// if the bytes are changing in the array, will affect this slice also.
	Bytes() []byte
	Len() int
}

func hashRep(h Hashable, w BWriter) []byte {
	_, data := h.ToBeHashed(w)
	return data
}
...
func HashObj(h Hashable, w BWriter) Digest {
	return Hash(hashRep(h, w))
}
